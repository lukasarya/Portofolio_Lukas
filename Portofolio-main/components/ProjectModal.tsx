...
export default function ProjectModal({ title, screenshots, currentIndex, onClose, onPrev, onNext }: ProjectModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8">
      ...
      <div className="relative max-w-5xl w-full max-h-[90vh] flex flex-col bg-dark-800 rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-dark-800">
          <h3 className="font-semibold text-white truncate pr-4">{title}</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-lg hover:bg-dark-100 flex items-center justify-center shrink-0 transition-colors">
            <X className="w-5 h-5 text-dark-500" />
          </button>
        </div>

        <div className="flex-1 overflow-hidden flex flex-col sm:flex-row">
          <div className="relative flex-1 bg-dark-900 flex items-center justify-center min-h-[250px] sm:min-h-[400px]">
            <div className="w-full h-full flex items-center justify-center p-6">
              {imgSrc && !hasError ? (
                ...
                <div
                  className="w-full max-w-3xl aspect-video rounded-xl flex flex-col items-center justify-center text-white shadow-lg"
                  style={{
                    background: `linear-gradient(135deg, ${color}, ${color}dd)`,
                  }}
                >
                  <ImageIcon className="w-16 h-16 mb-4 opacity-60" />
                  <p className="text-lg font-medium opacity-90">{ss.caption}</p>
          ...
          <div className="sm:w-72 p-6 border-t sm:border-t-0 sm:border-l border-dark-800 overflow-y-auto">
            <p className="text-xs font-medium text-dark-400 uppercase tracking-wider mb-2">
              Screenshot {currentIndex + 1} / {screenshots.length}
            </p>
            <h4 className="text-lg font-semibold text-white mb-2">{ss.caption}</h4>
            <p className="text-sm text-dark-400 leading-relaxed text-justify">{ss.desc}</p>
          </div>
        </div>
      </div>
    </div>
